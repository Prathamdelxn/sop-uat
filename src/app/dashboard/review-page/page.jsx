...
//   // Password is already verified by the PasswordModal component
//   // We can proceed with the approval
//   if (!sopToApprove) return;
...
//     setSopStatuses(prev => ({
//       ...prev,
//       [sopToApprove._id]: 'Approved'
//     }));
    
//     setApprovalMessage(`Checklist "${sopToApprove.name}" has been approved successfully!`);
...
//   // Actual approval function
//   const handleApprove = async () => {
//     if (!sopToApprove) return;
    
//     try {
//       setApprovingSopId(sopToApprove._id);
//       setLoading(true);
...
//                     <span className="font-medium">{task.minTime}</span>
//                   </div>
//                 )}
//                 {task.maxTime && !isZeroTime(task.maxTime) && (
//                   <div>
//                     <span className="text-gray-600">Maximum Duration: </span>
//                     <span className="font-medium">{task.maxTime}</span>
//                   </div>
//                 )}
//               </div>
//             </div>
...
//         >
//           <div 
//             className="relative bg-white rounded-xl shadow-xl w-full max-w-6xl max-h-[80vh] overflow-y-auto mx-4"
...
//                 </button>
//                 <button
//                   onClick={() => handleApproveClick(selectedSop)}
...
//                 <strong className="text-gray-900">{sopToApprove?.name}</strong>?
//               </p>
...
const ReviewPage = () => {
  ...
  const handlePasswordConfirm = async (password) => {
    // Password is already verified by the PasswordModal component
    // We can proceed with the action based on actionType
  ...
  const handleRejectAction = async () => {
    if (!selectedSop || !rejectReason.trim()) return;
        ...
        >
          ...
          >
            ...
            {showActionButtons(selectedSop) && (
              <div className="sticky bottom-0 bg-white border-t p-4 flex justify-end gap-3">
                <button
                  onClick={() => handleRejectClick(selectedSop)}
                  className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 flex items-center gap-2"
                >
                  <X className="w-5 h-5" />
                  Reject
