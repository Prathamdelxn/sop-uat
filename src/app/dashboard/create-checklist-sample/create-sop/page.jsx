...
// import { Plus, Trash2, Clock, Image, ChevronDown, ChevronRight, X, Camera, Minus, AlertCircle, ArrowBigLeftDash, ArrowBigLeft, Copy, GripVertical } from 'lucide-react';
...
//     // Only allow drop if it's the same level (same parentPath)
...
//     // Only allow drop if it's the same level (same parentPath)
//     if (JSON.stringify(data.parentPath) === JSON.stringify(parentPath)) {
//       onDrop(data, { stageId, taskId: task.id, parentPath, index });
//     }
//   };
//   // const handleDrop = (e) => {
//   //   e.preventDefault();
...
//   return (
//      <div
//       className={`border-2 ${depthStyles.border} ${depthStyles.bg} rounded-2xl p-4 sm:p-6 mb-4 shadow-md hover:shadow-lg transition-all duration-300 relative ${isDragging && dragIndex === index ? 'opacity-50' : ''}`}
//       draggable
...
//       {/* Drag handle */}
//       <div className="absolute top-4 left-2 cursor-grab active:cursor-grabbing text-gray-400 hover:text-gray-600">
...
//     // Only allow reordering if it's the same stage and same parent path
//     if (source.stageId === target.stageId && 
//         JSON.stringify(source.parentPath) === JSON.stringify(target.parentPath)) {
      
//       setStages(prev => prev.map(stage => {
//         if (stage.id === source.stageId) {
//           // Get the tasks array to modify
//           let tasksToModify = stage.tasks;
//           let path = [...source.parentPath];
          
//           // If we're dealing with subtasks, navigate to the correct level
//           if (path.length > 0) {
//             const findAndUpdateSubtasks = (tasks, pathIndex) => {
//               return tasks.map(task => {
//                 if (task.id === path[pathIndex]) {
//                   if (pathIndex === path.length - 1) {
//                     // We've reached the target level, reorder the subtasks
//                     const newSubtasks = [...task.subtasks];
//                     const [movedItem] = newSubtasks.splice(source.index, 1);
//                     newSubtasks.splice(target.index, 0, movedItem);
//                     return { ...task, subtasks: newSubtasks };
//                   } else {
//                     // Continue navigating
//                     return {
//                       ...task,
//                       subtasks: findAndUpdateSubtasks(task.subtasks, pathIndex + 1)
//                     };
//                   }
//                 }
//                 return task;
...
//             tasksToModify = findAndUpdateSubtasks(tasksToModify, 0);
//           } else {
//             // Reorder top-level tasks
//             const newTasks = [...tasksToModify];
//             const [movedItem] = newTasks.splice(source.index, 1);
//             newTasks.splice(target.index, 0, movedItem);
//             tasksToModify = newTasks;
//           }
          
//           return { ...stage, tasks: tasksToModify };
//         }
//         return stage;
//       }));
//     }
...
//       onToggleExpansion={toggleExpansion}
//       expandedItems={expandedItems}
//       taskNumber={`${stages.findIndex(s => s.id === selectedStage) + 1}.${index + 1}`}
//       hasDuplicateTitle={hasDuplicateTitle(selectedStage, task.id, [])}
//       onDragStart={handleDragStart}
//       onDragOver={handleDragOver}
//       onDrop={handleDrop}
...
// ... (ImageAttachmentModal and DurationModal components remain the same) ...
const ImageAttachmentModal = ({
  onClose,
  onSave,
  initialTitle = '',
...
const TaskComponent = React.memo(({
 ...
 const handleDragStart = (e) => {
  ...
  e.dataTransfer.effectAllowed = 'move';
  onDragStart(index, [...parentPath], level);
};
const handleDragOver = (e) => {
  e.preventDefault();
  e.dataTransfer.dropEffect = 'move';
...
useEffect(() => {
 ...
 const handleDrop = (e) => {
  ...
  try {
    ...
    // Only allow drop if it's the same level and same parent path
    if (JSON.stringify(data.parentPath) === JSON.stringify(parentPath) && 
        data.level === level) {
      onDrop(data, { 
        stageId, 
...
const validateAllTasks = (tasks) => {
  ...
  const handleDragStart = useCallback((index, parentPath, level) => {
    setDragIndex(index);
    setIsDragging(true);
  }, []);

  const handleDragOver = useCallback((index, parentPath, level) => {
    // Visual feedback can be added here if needed
  }, []);
  ...
  const handleDrop = useCallback((source, target) => {
    ...
    if (source.stageId === target.stageId && 
      ...
      setStages(prev => prev.map(stage => {
        if (stage.id === source.stageId) {
          // Recursive function to find and reorder tasks at any level
          const reorderTasks = (tasks, path, sourceIndex, targetIndex) => {
            // If we're at the target level (path is empty), reorder the tasks
            if (path.length === 0) {
              const newTasks = [...tasks];
            ...
            // Otherwise, navigate deeper into the subtasks
            return tasks.map(task => {
              if (task.id === path[0]) {
                return {
                  ...task,
                  subtasks: reorderTasks(task.subtasks || [], path.slice(1), sourceIndex, targetIndex)
                };
              }
              return task;
            });
          ...
          return {
            ...stage,
            tasks: reorderTasks(stage.tasks, source.parentPath, source.index, target.index)
          };
        }
        return stage;
      }));
