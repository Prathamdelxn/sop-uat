...
//   useEffect(() => {
//     if (hasSubtasks && expandedItems[task._id] === undefined) {
//       onToggleExpansion(task._id);
//     }
//   }, [hasSubtasks, task._id, expandedItems, onToggleExpansion]);
...
//     onUpdateTask(stageId, task._id, field, value, parentPath);
//   };

//   const handleImageSave = (imageData) => {
//     onUpdateTask(stageId, task._id, 'attachedImages', imageData.photos, parentPath);
//     onUpdateTask(stageId, task._id, 'imageTitle', imageData.title, parentPath);
//     onUpdateTask(stageId, task._id, 'imageDescription', imageData.description, parentPath);
//   };

//   const handleDurationSave = (duration) => {
//     onUpdateTask(stageId, task._id, 'minDuration', duration.minDuration, parentPath);
//     onUpdateTask(stageId, task._id, 'maxDuration', duration.maxDuration, parentPath);
//     onUpdateTask(stageId, task._id, 'minTime', duration.minTime, parentPath);
//     onUpdateTask(stageId, task._id, 'maxTime', duration.maxTime, parentPath);
//   };
...
//   const isExpanded = expandedItems[task._id] || false;

//   return (
//     <div className={`border-2 ${depthStyles.border} ${depthStyles.bg} rounded-2xl p-4 sm:p-6 mb-4 shadow-md hover:shadow-lg transition-all duration-300`}>
//       {showImageModal && (
...
//           {hasSubtasks && (
//             <button 
//               onClick={() => onToggleExpansion(task._id)} 
...
//         </div>
//         <button 
//           onClick={() => onDeleteTask(stageId, task._id, parentPath)} 
...
//           <button
//             onClick={() => onAddSubtask(stageId, task._id, parentPath)}
...
//               stageId={stageId}
//               level={level + 1}
//               parentPath={[...parentPath, task._id]}
...
//           setSelectedStage(data.stages[0]._id);
//         }
//       } catch (error) {
...
//       setSelectedStage(stages.length > 1 ? stages[0]._id : null);
//     }
...
//         userId: userData.id
//       };
// console.log("hello updated values",requestData);
...
//           <p className="mt-4 text-gray-700">Loading checklist...</p>
//         </div>
//       </div>
//     );
//   }
...
//               ></path>
//             </svg>
//             <p className="text-gray-700 font-medium">Updating Checklist...</p>
//           </div>
//         </div>
//       )}
...
//         <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mt-6 mb-4 sm:mb-6 tracking-tight">Edit Checklist</h1>
...
//                     {stages.find(s => s._id === selectedStage)?.name || 'Tasks'}
//                   </h2>
//                   <button
...
//                   {stages.find(s => s._id === selectedStage)?.tasks.map((task, index) => (
//                     <TaskComponent
//                       key={task._id}
//                       task={task}
...
//                       expandedItems={expandedItems}
//                       taskNumber={`${stages.findIndex(s => s._id === selectedStage) + 1}.${index + 1}`}
//                     />
//                   ))}
//                 </div>

//                 {stages.find(s => s._id === selectedStage)?.tasks.length === 0 && (
...
import React, { useState, useCallback, useMemo, useRef, useEffect } from 'react';
import { ObjectId } from 'bson';
import { Plus, Trash2, Clock, Image, ChevronDown, ChevronRight, X, Camera, Minus, AlertCircle, Copy } from 'lucide-react';
import { ArrowLeft } from 'react-feather';
import { ToastContainer, toast } from 'react-toastify';
import Link from 'next/link';
import { useRouter, useParams } from 'next/navigation';
    ...
    <div className={`border-2 ${depthStyles.border} ${depthStyles.bg} rounded-2xl p-4 sm:p-6 mb-4 shadow-md hover:shadow-lg transition-all duration-300`}>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <button
            onClick={() => onCopyTask(stageId, task._id, parentPath)}
...
const EditChecklistPage = () => {
  ...
  const copyTask = useCallback((stageId, taskId, parentPath = []) => {
    ...
    const insertCopiedTask = (tasks, targetId, copiedTask) => {
      ...
      const targetIndex = newTasks.findIndex(task => task._id === targetId);

      if (targetIndex !== -1) {
        // Insert the copy right after the original task
        const newTask = {
          ...JSON.parse(JSON.stringify(copiedTask)),
          _id: new ObjectId().toString(), // New unique ID
        };
  ...
  const checkForDuplicateTitles = useCallback(() => {
    ...
    const checkTasksForDuplicates = (tasks, levelPath = []) => {
      ...
      tasks.forEach(task => {
        const taskKey = [...levelPath, task._id].join('-');
        ...
        if (task.subtasks && task.subtasks.length > 0) {
          const subtaskDuplicates = checkTasksForDuplicates(task.subtasks, [...levelPath, task._id]);
    ...
    stages.forEach(stage => {
      if (stage.tasks && stage.tasks.length > 0) {
        const stageDuplicates = checkTasksForDuplicates(stage.tasks, [stage._id]);
                ...
                <div className="space-y-4 sm:space-y-6">
                  {stages.find(s => s._id === selectedStage)?.tasks.map((task, index) => (
                    <TaskComponent
                      ...
                      expandedItems={expandedItems}
                      taskNumber={`${stages.findIndex(s => s._id === selectedStage) + 1}.${index + 1}`}
                      hasDuplicateTitle={hasDuplicateTitle(selectedStage, task._id, [])}
                    />
                  ))}
                </div>
