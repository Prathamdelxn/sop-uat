...
import { Plus, Trash2, Clock, Image,ArrowLeft, ChevronDown, ChevronRight, X, Camera, Minus } from 'lucide-react';
import Link from 'next/link';
import { useRouter } from 'next/navigation';

// Copy all the modal components (ImageAttachmentModal, DurationModal) from your create-sop page
// Copy the TaskComponent from your create-sop page
...
const SopEditor = ({ 
  ...
  const [prototypeName, setPrototypeName] = useState(initialName);
  ...
  // Initialize selected stage when stages change
  useEffect(() => {
  ...
  // ... Copy all the stage/task management functions from your create-sop page:
  // addStage, updateStageName, addTask, addSubtask, updateTaskField, 
  // deleteTask, deleteStage, toggleExpansion, validatePrototype
 ...
 const updateStageName = useCallback((stageId, newName) => {
  ...
  const handleSavePrototype = async () => {
    ...
    try {
      ...
      // Call the passed onSave function with the data
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 p-4 sm:p-6 md:p-8">
      <div className="max-w-full mx-auto">
        <button 
          onClick={onCancel}
          className="px-4 py-2 bg-white rounded-lg border border-gray-300 hover:bg-gray-50 transition-colors duration-200 shadow-sm hover:shadow-md mb-6"
        >
        ...
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mt-6 mb-8 sm:mb-10 tracking-tight">
          {isEditing ? 'Edit Checklist' : 'Create New Checklist'}
        </h1>

        {/* Keep all the existing UI from your create-sop page */}
